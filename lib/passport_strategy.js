...
function OpenIDConnectStrategy(
  ...
  if (this._usePKCE === true) {
    ...
    assert(
      ...
      "issuer code_challenge_methods_supported is empty"
    );
    if (supportedMethods.includes("S256")) {
      ...
      throw new Error(
        "neither S256 or plain code_challenge_method is supported by the issuer"
      );
    }
  ...
  if (!params.response_type)
    params.response_type = _.get(client, "response_types[0]", "code");
  if (!params.redirect_uri)
    params.redirect_uri = _.get(client, "redirect_uris[0]"); // TODO: only default if there's one
...
OpenIDConnectStrategy.prototype.authenticate = function authenticate(
  ...
  let redirectUrl = this._params;
  try {
    if (!req.session) {
      throw new Error(
        "authentication requires session support when using state, max_age or nonce"
      );
    }
    ...
    if (_.isEmpty(reqParams)) {
      ...
      if (!params.redirect_uri.includes("http")) {
        ...
        params.redirect_uri = protocol + host + params.redirect_uri;

        console.log("with req", host, protocol, params);
      }
      ...
      if (this._usePKCE) {
        ...
        switch (
          ...
          case "S256":
            params.code_challenge = base64url.encode(
              crypto
                .createHash("sha256")
                .update(verifier)
                .digest()
    ...
    if (!this._params.redirect_uri.includes("http")) {
      const host = req.headers.host;
      const protocol = host === "localhost:3000" ? "http://" : "https://";
      this._params.redirect_uri = protocol + host + this._params.redirect_uri;

      console.log("without req", host, protocol, this._params);
    }
    ...
    let callback = client
      .authorizationCallback(opts.redirect_uri, reqParams, checks)
      .then(tokenset => {
